...
export default function NexusMirrorModal({ onClose }: NexusMirrorModalProps) {
  ...
  const runCrossQuestioning = async (item: InterviewQAItem) => {
    ...
    try {
      ...
      setPressureMeter((prev) => Math.min(10, prev + 1));
      setError(null);
      addNexusActivityEntry({
        agentType: 'mirror',
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-6 pointer-events-auto overflow-hidden">
      ...
      <div className="relative w-full max-w-5xl bg-white rounded-lg shadow-xl p-6 md:p-8 border border-slate-200 max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="mb-4 flex items-center gap-3">
          <button
            ...
            className="px-4 py-2 bg-green-600 hover:bg-green-700 disabled:opacity-40 text-white rounded-md text-[10px] font-black uppercase tracking-wider inline-flex items-center gap-2"
          >
        ...
        <div className="mb-4 border border-zinc-100 rounded-md bg-zinc-50/60 p-3">
        ...
        <div className="flex-1 overflow-auto space-y-3 pr-1">
          {loading && nexusMirrorItems.length === 0 ? (
            <div className="space-y-3">
              {[1, 2, 3].map((i) => (
                <div key={i} className="rounded-md border border-zinc-100 bg-zinc-50/30 p-4 animate-pulse">
                  <div className="h-3 bg-zinc-200 rounded w-16 mb-4"></div>
            ...
            <div className="h-full min-h-48 border border-dashed border-zinc-200 rounded-md flex items-center justify-center bg-zinc-50/40">
            ...
            nexusMirrorItems.map((item) => (
              <div key={item.id} className="rounded-md border border-zinc-100 bg-zinc-50/30 p-4">
