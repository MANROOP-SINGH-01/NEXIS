...
export function FinalOutputModal() {
  ...
  const renderContent = () => {
        ...
        <div className="space-y-4">
          <div className="relative group">
            <img
              ...
              className="w-full rounded-lg shadow-xl border border-black/5"
            />
            <button
              onClick={handleDownload}
              className="absolute top-4 right-4 p-2 bg-white/90  rounded-full shadow-lg border border-black/5 opacity-0 group-hover:opacity-100 transition-opacity hover:bg-white text-zinc-600 active:scale-95"
    ...
    if (finalAssetType === 'audio' && finalAssetContent) {
      return (
        <div className="space-y-4">
          <div className="flex flex-col sm:flex-row items-center gap-3 p-3 bg-white border border-zinc-100 rounded-lg shadow-sm">
    ...
    if (finalAssetType === 'video' && finalAssetContent) {
      return (
        <div className="space-y-4">
          <div className="relative group">
            <video controls className="w-full rounded-lg shadow-xl border border-black/5">
              <source src={finalAssetContent} type="video/mp4" />
              Your browser does not support the video tag.
            </video>
            <button
              onClick={handleDownload}
              className="absolute top-4 right-4 p-2 bg-white/90  rounded-full shadow-lg border border-black/5 opacity-0 group-hover:opacity-100 transition-opacity hover:bg-white text-zinc-600 active:scale-95 z-10"
  ...
  return (
    ...
    >
      <div
        className="bg-zinc-50 border border-black/10 rounded-lg w-180 max-w-full max-h-[90vh] flex flex-col shadow-2xl overflow-hidden"
      ...
      >
        ...
        <div className="flex items-center justify-between px-8 py-6 border-b border-black/5 bg-white">
          ...
          <button onClick={() => setFinalOutputOpen(false)} className="w-8 h-8 flex items-center justify-center rounded-md border border-slate-200 hover:bg-slate-100 text-slate-500 hover:text-slate-800 transition-colors cursor-pointer" title="Close"><X className="w-4 h-4" /></button>
        </div>
        ...
        <div className="px-8 py-6 border-t border-black/5 flex flex-col gap-6 bg-white">
          ...
          <div className="flex justify-between items-center">
            ...
            <button
              onClick={handleCopy}
              className="px-6 py-3 bg-darkDelegation text-white rounded-lg text-[10px] font-black uppercase tracking-[0.15em] hover:bg-black active:scale-[0.98] transition-all shadow-lg shadow-black/10"
            >
