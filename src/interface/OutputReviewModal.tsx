...
export function OutputReviewModal() {
  ...
  return (
    ...
    >
      <div
        className="bg-white border border-black/10 rounded-lg w-180 max-w-full max-h-[90vh] flex flex-col shadow-2xl overflow-hidden scale-in"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-8 py-6 border-b border-zinc-100 bg-white">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-lg bg-darkDelegation flex items-center justify-center text-white shadow-lg">
              <Icon size={24} />
            </div>
            <div>
        ...
        <div className="flex-1 overflow-y-auto px-8 py-8 space-y-8 bg-zinc-50/30">
          ...
          <div className="space-y-3">
            ...
            <textarea
              ...
              className="w-full h-40 bg-white border border-zinc-200 rounded-lg p-4 text-sm text-zinc-700 leading-relaxed font-sans focus:ring-2 focus:ring-darkDelegation outline-none resize-none shadow-sm"
        ...
        <div className="px-8 py-6 border-t border-zinc-100 flex justify-end items-center bg-white gap-3">
          <button
            onClick={handleCancelAndReset}
            className="px-6 py-3 bg-white border border-zinc-200 text-zinc-500 rounded-lg text-[10px] font-black uppercase tracking-[0.15em] hover:bg-zinc-50 hover:text-red-500 hover:border-red-500 transition-all active:scale-[0.98]"
          >
          ...
          <button
            ...
            className="px-8 py-3 bg-darkDelegation text-white rounded-lg text-[10px] font-black uppercase tracking-[0.15em] hover:bg-black active:scale-[0.98] transition-all shadow-lg shadow-black/10 flex items-center gap-2"
          >
      ...
      {isConfirmingReset && (
        <div
          className="fixed inset-0 z-[120] flex items-center justify-center bg-slate-900/60  p-4 cursor-default"
          onClick={(e) => {
            e.stopPropagation()
            setIsConfirmingReset(false)
          }}
          ...
          >
            ...
            <div className="flex flex-col w-full gap-2 mt-2">
              <button
                ...
                className="w-full py-4 bg-red-500 text-white rounded-lg text-[10px] font-black uppercase tracking-[0.15em] hover:bg-red-600 active:scale-[0.98] transition-all"
              >
                Yes, Reset Project
              </button>
              <button
                onClick={() => setIsConfirmingReset(false)}
                className="w-full py-4 bg-zinc-100 text-zinc-500 rounded-lg text-[10px] font-black uppercase tracking-[0.15em] hover:bg-zinc-200 active:scale-[0.98] transition-all"
              >
                No, Go Back
              </button>
            </div>
