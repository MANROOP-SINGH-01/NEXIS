...
export default function PhaseOneControlPanel() {
  ...
  return (
    <div className="px-4 py-2.5 border-b border-zinc-100 bg-white shrink-0">
      <div className="grid grid-cols-1 md:grid-cols-12 gap-2.5 items-stretch">
        <div className="md:col-span-5 min-h-24 bg-zinc-50 border border-zinc-200 rounded-md px-3 py-2 text-xs flex flex-col justify-between">
          <input
            ref={fileRef}
            type="file"
        ...
        <textarea
          ...
          className="md:col-span-4 min-h-24 bg-zinc-50 border border-zinc-200 rounded-md px-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-darkDelegation/20 resize-none"
        />
        ...
        <div className="md:col-span-3 flex flex-col gap-1.5 justify-center">
          <button
            ...
            className="h-full min-h-16 bg-blue-700 hover:bg-blue-800 disabled:opacity-50 text-white rounded-md text-xs font-bold uppercase tracking-wider transition-colors shadow-none active:scale-[0.99] flex items-center justify-center gap-2.5 cursor-pointer disabled:cursor-not-allowed group"
          ...
          {!canRun && !loading && (
            <div className="text-[9px] font-medium text-amber-600/90 text-center leading-tight bg-amber-50/80 border border-amber-200/50 rounded-lg py-1 px-1.5">
              <span className="inline-flex items-center gap-1 justify-center"><AlertTriangle size={11} className="shrink-0 text-amber-600" /> {!resumeLen ? "Upload Resume PDF" : resumeLen <= 30 ? `Resume too short (${resumeLen}/30)` : `Paste JD (>30 chars, now ${jdLen})`}</span>
            </div>
          )}
