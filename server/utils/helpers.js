...
export function inferJobMetaFromLink(link) {
  ...
  const isAdzuna = l.includes('adzuna.com')
  return {
    source: isLinkedIn ? 'linkedin' : isDirectCareers ? 'company-careers' : isAdzuna ? 'adzuna' : 'hidden',
    competitionLevel: isLinkedIn ? 'High' : isDirectCareers ? 'Low' : isAdzuna ? 'Low' : 'Medium',
    blueOceanBoost: isLinkedIn ? -12 : isDirectCareers ? 12 : isAdzuna ? 10 : 4,
  }
}
